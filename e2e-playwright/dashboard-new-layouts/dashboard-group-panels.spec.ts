...
test.skip('Grouping panels', () => {
  ...
  test('can group and ungroup new panels into row', async ({ dashboardPage, selectors, page }) => {
    ...
    await expect(dashboardPage.getByGrafanaSelector(selectors.components.DashboardRow.title('New row'))).toBeVisible();
  ...
  test('can add multiple rows and ungroup them all at once', async ({ dashboardPage, selectors, page }) => {
    ...
    await expect(dashboardPage.getByGrafanaSelector(selectors.components.Panels.Panel.title('New panel'))).toHaveCount(
      4
    ); // 3 from first row + 1 from third row
    ...
    await expect(dashboardPage.getByGrafanaSelector(selectors.components.Panels.Panel.title('New panel'))).toHaveCount(
      4
    ); // All 4 panels should be visible in the single grid
  ...
  test('can paste a copied row', async ({ dashboardPage, selectors, page }) => {
    ...
    await dashboardPage.getByGrafanaSelector(selectors.components.CanvasGridAddActions.addRow).scrollIntoViewIfNeeded();
