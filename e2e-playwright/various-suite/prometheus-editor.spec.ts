...
test.describe('Prometheus query editor', () => {
  ...
  test('should have a kickstart component', async ({ createDataSource, page, selectors, gotoDataSourceConfigPage }) => {
  ...
  test('should have an explain component', async ({ createDataSource, gotoDataSourceConfigPage, page, selectors }) => {
    const DATASOURCE_NAME = `prometheus_${Date.now()}`;
    await navigateToEditor(page, selectors, createDataSource, 'Code', DATASOURCE_NAME, gotoDataSourceConfigPage);
  ...
  test.describe('Code editor', () => {
    ...
    test('navigates to the code editor and opens the metrics browser with metric search, labels, label values, and all components', async ({
      ...
      const clear = page.getByTestId(selectors.components.DataSource.Prometheus.queryEditor.code.metricsBrowser.clear);
