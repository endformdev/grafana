...
test.describe('Migrate to Cloud (On-prem)', () => {
  test.describe('with mocked calls to the API backend', () => {
    ...
    test('creates and uploads a snapshot successfully', async ({ page }) => {
      ...
      const includeAllCheckbox = page.getByTestId('migrate-to-cloud-configure-snapshot-checkbox-resource-include-all');
      ...
      const muteTimingCheckbox = page.getByTestId('migrate-to-cloud-configure-snapshot-checkbox-resource-mute_timing');
