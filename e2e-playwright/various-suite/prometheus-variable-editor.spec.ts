...
test.describe('Prometheus variable query editor', () => {
  ...
  async function variableFlowToQueryEditor(
    ...
    // Select query type
    const queryTypeSelect = page.getByTestId(selectors.components.DataSource.Prometheus.variableQueryEditor.queryType);
    ...
    const backToDashboardButton = page.getByTestId(selectors.components.NavToolbar.editDashboard.backToDashboardButton);
  ...
  test('should select a query type for a Prometheus variable query', async ({ createDataSource, page, selectors }) => {
