...
test.describe('Azure Monitor datasource', () => {
  ...
  test('create dashboard, add panel for metrics, log analytics, ARG, and traces queries', async ({
    ...
    codeEditor = resourceGraphPanel.getByGrafanaSelector(selectors.components.CodeEditor.container).locator('textarea');
  ...
  test('create dashboard with template variables', async ({
    ...
    await addAzureMonitorVariable(page, dashboardPage, selectors, 'namespaces', AzureQueryType.NamespacesQuery, false, {
      subscription: '$subscription',
  ...
  async function addAzureMonitorVariable(
    ...
    const queryTypeInput = page.getByTestId(azMonSelectors.components.variableEditor.queryType.input).locator('input');
    ...
    switch (type) {
      ...
      case AzureQueryType.ResourceNamesQuery:
        ...
        if (options?.region) {
          const regionInput = page.getByTestId(azMonSelectors.components.variableEditor.region.input).locator('input');
