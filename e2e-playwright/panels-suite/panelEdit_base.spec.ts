...
test.describe('Panels test: Panel edit base', () => {
  test('Tests various Panel edit scenarios', async ({ gotoDashboardPage, selectors, page }) => {
    ...
    await expect(
      dashboardPage.getByGrafanaSelector(selectors.components.PanelEditor.OptionsPane.fieldLabel('Table Column width'))
    ).toBeVisible();
  });
});
